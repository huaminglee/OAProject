...
namespace ZWL.Common
{
    ...
    public class DataToExcel
    {
        ...
        public static void ExportToExcel(string filename, string content)
        {
            ...
            //content = String.Format("<style type='text/css'>{0}</style>{1}", css, content);

            res.Clear();
            res.Buffer = true;
            res.Charset = "UTF-8";
            res.AddHeader("Content-Disposition", "attachment;filename=" + System.Web.HttpUtility.UrlEncode(filename,System.Text.Encoding.UTF8));
