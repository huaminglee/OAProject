...
namespace ZWL.BLL
{
    ...
    public class ERPUser
    {
        ...
        public int Add()
        {
            ...
            parameters[41].Value = Cardno;
            parameters[42].Value = Tel;
        ...
        public void GetModel(int ID)
        {
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                Cardno = ds.Tables[0].Rows[0]["CardNo"].ToString();
        ...
        public List<ERPUser> GetListByJueSe(string juese)
        {
            if (juese.Contains(","))
            {
                juese = juese.Replace(",", "','");
            ...
            StringBuilder strSql = new StringBuilder();
            strSql.AppendFormat("select [ID],[UserName] FROM ERPUser Where JiaoSe in ('{0}')", juese);

            DataSet ds = DbHelperSQL.Query(strSql.ToString());
            List<ERPUser> list = new List<ERPUser>();
            if (ds.Tables[0].Rows.Count > 0)
            {
                foreach (DataRow dr in ds.Tables[0].Rows)
                {
                    list.Add(new ERPUser()
                    {
