            ...
            <tr>
                <td colspan="2" style="border-bottom: #006633 1px dashed; height: 30px" valign="middle">
                    &nbsp; &nbsp; 用户名：<asp:TextBox ID="TextBox1" runat="server" Height="20px" Width="80px"></asp:TextBox><img
                        ...
                        src="../images/Button/search.gif" />&nbsp; 起至日期：<input class="Wdate" type="text"
                            autocomplete="off" id="TextBox2" runat="server" onclick="WdatePicker({dateFmt:'yyyy-MM-dd'})" />
                    ~
                    ...
                    &nbsp; &nbsp;<span style="color: #0000cc">注：必须且只能选择单一用户查询明细</span>
                </td>
            </tr>
        </table>
    </div>
            ...
            <td>
                <asp:GridView ID="GVData" runat="server" AllowPaging="True" AllowSorting="True" AutoGenerateColumns="False"
                    ...
                    <Columns>
                        ...
                        <asp:BoundField DataField="KaoQinRiQi" HeaderText="考勤日期" DataFormatString="{0:yyyy-MM-dd}"
                            HtmlEncode="False"></asp:BoundField>
                        <asp:TemplateField HeaderText="第一次登记(上班)">
                            <ItemTemplate>
                                <asp:Label ID="Label1" Text='<%# DataBinder.Eval(Container.DataItem, "DengJiTime1") %>'
                                    ToolTip='<%# DataBinder.Eval(Container.DataItem, "GuiDingTime1") %>' runat="server"></asp:Label>
                            </ItemTemplate>
                        </asp:TemplateField>
                        <asp:TemplateField HeaderText="第二次登记(下班)">
                            <ItemTemplate>
                                <asp:Label ID="Label2" Text='<%# DataBinder.Eval(Container.DataItem, "DengJiTime2") %>'
                                    ToolTip='<%# DataBinder.Eval(Container.DataItem, "GuiDingTime2") %>' runat="server"></asp:Label>
                            </ItemTemplate>
                        </asp:TemplateField>
                        <asp:TemplateField HeaderText="第三次登记(上班)">
                            <ItemTemplate>
                                <asp:Label ID="Label3" Text='<%# DataBinder.Eval(Container.DataItem, "DengJiTime3") %>'
                                    ToolTip='<%# DataBinder.Eval(Container.DataItem, "GuiDingTime3") %>' runat="server"></asp:Label>
                            </ItemTemplate>
                        </asp:TemplateField>
                        <asp:TemplateField HeaderText="第四次登记(下班)">
                            <ItemTemplate>
                                <asp:Label ID="Label4" Text='<%# DataBinder.Eval(Container.DataItem, "DengJiTime4") %>'
                                    ToolTip='<%# DataBinder.Eval(Container.DataItem, "GuiDingTime4") %>' runat="server"></asp:Label>
                            </ItemTemplate>
                        </asp:TemplateField>
                        <asp:TemplateField HeaderText="第五次登记(上班)">
                            <ItemTemplate>
                                <asp:Label ID="Label5" Text='<%# DataBinder.Eval(Container.DataItem, "DengJiTime5") %>'
                                    ToolTip='<%# DataBinder.Eval(Container.DataItem, "GuiDingTime5") %>' runat="server"></asp:Label>
                            </ItemTemplate>
                        </asp:TemplateField>
                        <asp:TemplateField HeaderText="第六次登记(下班)">
                            <ItemTemplate>
                                <asp:Label ID="Label6" Text='<%# DataBinder.Eval(Container.DataItem, "DengJiTime6") %>'
                                    ToolTip='<%# DataBinder.Eval(Container.DataItem, "GuiDingTime6") %>' runat="server"></asp:Label>
                            </ItemTemplate>
                        </asp:TemplateField>
                    </Columns>
